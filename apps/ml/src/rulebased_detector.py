...
class GolfNpyDetector:
    ...
    def detect_phases(self, keypoints):
        ...
        velocity = np.linalg.norm(diff, axis=2).mean(
            axis=1
        )  # Avg velocity of all joints
...
def process_npy_folder(folder_path, output_csv):
    ...
    for npy_path in npy_files:
        ...
        try:
            ...
            # if vid_name:
            #     visualize_video(
            #         video_path=f"data/TDTU_raw/{vid_name}.mp4",
            #         output_path=f"data/debug_rule/debug_{vid_name}.mp4",
            #         events=phases,
            #     )
