...
INPUT_FOLDER = ROOT / 'data' / 'TDTU-Golf-Pose-v1' / 'preprocessed'  # Folder chứa video gốc (0.mp4, 1.mp4...)
OUTPUT_FOLDER = ROOT / 'data' / 'TDTU-Golf-Pose-v1' / 'preprocessed_vitpose_npy'  # Folder lưu kết quả
CONF_THRESHOLD = 0.5  # Độ tin cậy tối thiểu để chọn người
MAX_WORKERS = 6  # Số luồng chạy song song (Giảm bớt vì ViTPose tốn VRAM hơn YOLO)
    ...
    try:
        ...
        while True:
            ...
            if best_box is not None:
                ...
                # Expand the box slightly (Padding) to ensure limbs are fully included and provide context
                # Standard practice for Top-Down Pose Estimation is 1.25x expansion
                pad_scale = 1.25
                box_w = x2 - x1
                box_h = y2 - y1
                ...
                new_w = box_w * pad_scale
                new_h = box_h * pad_scale
                ...
                if x2 > x1 and y2 > y1:
                    ...
                    # The processor REQUIRES a 'boxes' argument [x, y, w, h].
                    # Since we are passing the CROPPED image, the box of the person effectively covers the image.
                    # We pass a local box starting at 0,0 to satisfy the API.
                    local_box = [[0, 0, crop_w, crop_h]]
                    
                    # We also prepare the global box to map the coordinates back to the original full frame later.
                    global_box = [[x1, y1, crop_w, crop_h]]

                    # Pass local_box to processor. It will strictly crop/resize based on this.
                    # Since it matches the image size, it acts as a pass-through (with resizing).
                    inputs = processor(images=pil_img, boxes=[local_box], return_tensors="pt")
