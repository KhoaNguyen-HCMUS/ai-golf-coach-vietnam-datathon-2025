...
export const processVideoWithHits = async (
  videoPath,
  hits,
  ...
  let timestamp = providedTimestamp;
  if (!timestamp) {
    const filename = path.basename(videoPath);
    const timestampMatch = filename.match(/video_(\d+)_/);
    timestamp = timestampMatch ? timestampMatch[1] : Date.now().toString();
  }

  console.log(`[Video] 🎯 Using timestamp: ${timestamp}`);
